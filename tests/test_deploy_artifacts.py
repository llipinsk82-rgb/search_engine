...
class DeployArtifactTests(unittest.TestCase):
    ...
    def test_production_deploy_has_backup_and_rollback_guards(self):
        ...
        self.assertIn("required = {'xvideos', 'xnxx', 'sunporno', 'xgroovy', 'txxx', 'porndig', 'justporn', 'fpo', 'bigassporn', 'brazzilmoms', 'sextubespot', 'xcafe', 'mypornhere', 'pussyspace', 'tubev', 'xxxbule', 'theyarehuge', 'sexvid', 'pornid', 'zbporn', 'megatube', 'freeporn', 'pornsexvideo', 'lexotic', 'porndoe'}", t)
    ...
    def test_shipped_provider_catalog_is_incremental(self):
        rows=json.loads((ROOT/'deploy'/'search-engine-providers.example.json').read_text()); self.assertEqual({r['name'] for r in rows},{'xvideos','xnxx','sunporno','xgroovy','txxx','porndig','justporn','fpo','bigassporn','brazzilmoms','sextubespot','xcafe','mypornhere','pussyspace','tubev','xxxbule','theyarehuge','sexvid','pornid','zbporn', 'megatube', 'freeporn', 'pornsexvideo', 'lexotic', 'porndoe'}); self.assertTrue(all(r['sync_mode']=='incremental' for r in rows))
