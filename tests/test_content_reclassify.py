...
def _seed(db: Path) -> None:
    ...
    with sqlite3.connect(db) as conn:
        rows = [
            ("a", "demo", "A", "https://example.com/a", '["homemade"]', None, "unknown", "none", 1),
            ("c", "demo", "C", "https://example.com/c", '["homemade","professional"]', "Studio C", "studio", "studio_label", 1),
            ("n", "demo", "N", "https://example.com/n", '["hd"]', None, "studio", "tag_studio", 1),
            ("s", "demo", "S", "https://example.com/s", '["production"]', None, "unknown", "none", 1),
            ("l", "demo", "L", "https://example.com/l", '["hd"]', "Label Studio", "unknown", "none", 1),
            ("z", "demo", "Z", "https://example.com/z", '["homemade"]', None, "unknown", "none", 0),
        ]
        conn.executemany(
            """
            INSERT INTO items(
        ...
        conn.execute(
            "INSERT INTO provider_state(provider,state_key,state_value) VALUES('demo','backfill_cursor','123')"
        )
...
def test_dry_run_predicts_changes_without_writing(tmp_path: Path) -> None:
    ...
    assert report.after == {"amateur": 1, "studio": 2, "unknown": 2}
...
def test_apply_is_idempotent_and_preserves_fts_and_provider_state(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(db) as conn:
        assert conn.execute(
            "SELECT state_value FROM provider_state WHERE provider='demo' AND state_key='backfill_cursor'"
        ).fetchone() == ("123",)
        assert conn.execute(
