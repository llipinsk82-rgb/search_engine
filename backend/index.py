...
def _refresh_fts_items(conn: sqlite3.Connection, items: list[SearchItem]) -> None:
    ...
    for offset in range(0, len(ids), chunk_size):
        ...
        conn.execute(f"DELETE FROM items_fts WHERE id IN ({placeholders})", chunk)
    conn.executemany(
        "INSERT INTO items_fts (id, title, tags, provider) VALUES (?, ?, ?, ?)",
        [(item.id, item.title, " ".join(item.tags), item.provider) for item in latest.values()],
    )


def _write_items(conn: sqlite3.Connection, entries: list[tuple[SearchItem, int]]) -> None:
...
def upsert_items(items: list[SearchItem], path: Path = DB_PATH) -> int:
    initialize(path)
    with _connect(path) as conn:
        _write_items(conn, [(item, source_order) for source_order, item in enumerate(items)])
    return len(items)


def merge_provider_batches(batches, *, path: Path = DB_PATH) -> int:
    initialize(path)
...
def merge_provider_items(provider: str, items: list[SearchItem], *, allow_empty: bool = False, path: Path = DB_PATH) -> ProviderSyncStats:
    ...
    if mismatched:
        raise ValueError(f"provider mismatch for {len(mismatched)} item(s); expected {provider!r}")
    initialize(path)
    with _connect(path) as conn:
        active_before = int(conn.execute("SELECT COUNT(*) AS n FROM items WHERE provider = ? AND active = 1", (provider,)).fetchone()["n"])
        _write_items(conn, [(item, source_order) for source_order, item in enumerate(items)])
        active_after = int(conn.execute("SELECT COUNT(*) AS n FROM items WHERE provider = ? AND active = 1", (provider,)).fetchone()["n"])
    return ProviderSyncStats(provider, len(items), active_before, active_after, 0)


def replace_provider_items(provider: str, items: list[SearchItem], *, allow_empty: bool = False, path: Path = DB_PATH) -> ProviderSyncStats:
    if not provider.strip():
        raise ValueError("provider cannot be empty")
    if not items and not allow_empty:
    ...
    initialize(path)
    with _connect(path) as conn:
        rows = conn.execute("SELECT id FROM items WHERE provider = ? AND active = 1", (provider,)).fetchall()
        previous_ids = {str(row["id"]) for row in rows}
        incoming_ids = {item.id for item in items}
        conn.execute("UPDATE items SET active = 0 WHERE provider = ? AND active = 1", (provider,))
        _write_items(conn, [(item, source_order) for source_order, item in enumerate(items)])
        conn.execute("DELETE FROM items_fts WHERE id IN (SELECT id FROM items WHERE provider = ? AND active = 0)", (provider,))
        active_after = int(conn.execute("SELECT COUNT(*) AS n FROM items WHERE provider = ? AND active = 1", (provider,)).fetchone()["n"])
    return ProviderSyncStats(provider, len(items), len(previous_ids), active_after, len(previous_ids - incoming_ids))
...
def count_items(path: Path = DB_PATH) -> int:
    initialize(path)
    with _connect(path) as conn:
        return int(conn.execute("SELECT COUNT(*) AS n FROM items WHERE active = 1").fetchone()["n"])


def indexed_providers(path: Path = DB_PATH) -> list[str]:
    initialize(path)
    with _connect(path) as conn:
        rows = conn.execute("SELECT DISTINCT provider FROM items WHERE active = 1 ORDER BY provider").fetchall()
...
def provider_counts(path: Path = DB_PATH) -> dict[str, int]:
    initialize(path)
    with _connect(path) as conn:
        rows = conn.execute("SELECT provider, COUNT(*) AS n FROM items WHERE active = 1 GROUP BY provider ORDER BY provider").fetchall()
        return {str(row["provider"]): int(row["n"]) for row in rows}


def _where_for_search(query: str, *, provider: str | None, allowed_providers: set[str] | None, quality: str | None, content_class: str | None, age_check: str | None, min_duration: int | None, max_duration: int | None, exclude_ids: set[str] | None = None) -> tuple[str, list[object], str, str]:
    where = ["i.active = 1"]
    params: list[object] = []
    joins = ""
...
def count_search_items(query: str, *, provider: str | None = None, quality: str | None = None, content_class: str | None = None, age_check: str | None = None, min_duration: int | None = None, max_duration: int | None = None, allowed_providers: set[str] | None = None, path: Path = DB_PATH) -> int:
    initialize(path)
    where, params, joins, _ = _where_for_search(query, provider=provider, allowed_providers=allowed_providers, quality=quality, content_class=content_class, age_check=age_check, min_duration=min_duration, max_duration=max_duration)
    with _connect(path) as conn:
        return int(conn.execute("SELECT COUNT(*) AS n FROM items i " + joins + " WHERE " + where, params).fetchone()["n"])


def get_item(item_id: str, path: Path = DB_PATH) -> SearchItem | None:
    initialize(path)
    with _connect(path) as conn:
        row = conn.execute("SELECT id, provider, title, url, thumbnail, preview_url, duration_seconds, published_at, views, rating_percent, rating_count, quality, content_class, studio, age_check_status, tags_json FROM items WHERE id = ? AND active = 1", (item_id,)).fetchone()
    if row is None:
        return None
    return SearchItem(id=row["id"], provider=row["provider"], title=row["title"], url=row["url"], thumbnail=row["thumbnail"], preview_url=row["preview_url"], duration_seconds=row["duration_seconds"], published_at=datetime.fromisoformat(row["published_at"]) if row["published_at"] else None, views=row["views"], rating_percent=row["rating_percent"], rating_count=row["rating_count"], quality=row["quality"], tags=json.loads(row["tags_json"] or "[]"), content_class=row["content_class"], studio=row["studio"], age_check_status=row["age_check_status"], score=0.0)


def update_item_thumbnail(item_id: str, thumbnail: str, *, path: Path = DB_PATH) -> bool:
    initialize(path)
    with _connect(path) as conn:
        return conn.execute("UPDATE items SET thumbnail = ? WHERE id = ? AND active = 1", (thumbnail, item_id)).rowcount == 1


def search_items(query: str, *, provider: str | None = None, quality: str | None = None, content_class: str | None = None, age_check: str | None = None, min_duration: int | None = None, max_duration: int | None = None, allowed_providers: set[str] | None = None, exclude_ids: set[str] | None = None, offset: int = 0, limit: int = 40, sort: SortMode = "relevance", path: Path = DB_PATH) -> list[SearchItem]:
    initialize(path)
    where, params, joins, rank_select = _where_for_search(query, provider=provider, allowed_providers=allowed_providers, quality=quality, content_class=content_class, age_check=age_check, min_duration=min_duration, max_duration=max_duration, exclude_ids=exclude_ids)
    tokens = _token_re.findall(query.lower())
    if sort == "relevance":
        order = "fts_rank ASC, i.indexed_at DESC, i.source_order ASC" if tokens else "i.indexed_at DESC, i.source_order ASC"
    else:
    ...
    sql = f"SELECT i.id, i.provider, i.title, i.url, i.thumbnail, i.preview_url, i.duration_seconds, i.published_at, i.views, i.rating_percent, i.rating_count, i.quality, i.content_class, i.studio, i.age_check_status, i.tags_json, {rank_select} FROM items i {joins} WHERE {where} ORDER BY {order} LIMIT ? OFFSET ?"
    ...
    for position, row in enumerate(rows):
        score = -float(row["fts_rank"] or 0.0) if tokens else 1.0 - (position / total_rows)
        result.append(SearchItem(id=row["id"], provider=row["provider"], title=row["title"], url=row["url"], thumbnail=row["thumbnail"], preview_url=row["preview_url"], duration_seconds=row["duration_seconds"], published_at=datetime.fromisoformat(row["published_at"]) if row["published_at"] else None, views=row["views"], rating_percent=row["rating_percent"], rating_count=row["rating_count"], quality=row["quality"], tags=json.loads(row["tags_json"] or "[]"), content_class=row["content_class"], studio=row["studio"], age_check_status=row["age_check_status"], score=score))
    return result


def get_provider_state(provider: str, state_key: str, *, path: Path = DB_PATH) -> str | None:
    initialize(path)
    with _connect(path) as conn:
        row = conn.execute("SELECT state_value FROM provider_state WHERE provider = ? AND state_key = ?", (provider, state_key)).fetchone()
        return None if row is None else str(row["state_value"])


def set_provider_state(provider: str, state_key: str, state_value: str, *, path: Path = DB_PATH) -> None:
    initialize(path)
    with _connect(path) as conn:
        conn.execute("INSERT INTO provider_state (provider, state_key, state_value, updated_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP) ON CONFLICT(provider, state_key) DO UPDATE SET state_value=excluded.state_value, updated_at=CURRENT_TIMESTAMP", (provider, state_key, state_value))


def delete_provider_state(provider: str, state_key: str, *, path: Path = DB_PATH) -> None:
    initialize(path)
    with _connect(path) as conn:
        conn.execute("DELETE FROM provider_state WHERE provider = ? AND state_key = ?", (provider, state_key))
