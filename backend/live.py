...
_ZZZTUBE_CARD_RE = re.compile(
    r'<div\b[^>]*class="[^"]*\bb-thumb-item\b[^"]*\bjs-thumb\b[^"]*"[^>]*>'
    r'(?P<body>.*?)(?=<div\b[^>]*class="[^"]*\bb-thumb-item\b[^"]*\bjs-thumb\b|\Z)',
    re.IGNORECASE | re.DOTALL,
)
_ZZZTUBE_ANCHOR_RE = re.compile(r'<a\b(?P<attrs>[^>]*)>', re.IGNORECASE | re.DOTALL)
_ZZZTUBE_URL_RE = re.compile(r'\bhref="(?P<url>/[0-9]+(?:\?[^"]*)?)"', re.IGNORECASE)
_ZZZTUBE_TITLE_RE = re.compile(r'\btitle="(?P<title>[^"]+)"', re.IGNORECASE)
_ZZZTUBE_THUMB_RE = re.compile(
    r'\bdata-src="(?P<thumb>(?:https?:)?//[^"]+)"', re.IGNORECASE
)
_ZZZTUBE_PREVIEW_RE = re.compile(r'\bdata-preview="(?P<preview>[^"]*)"', re.IGNORECASE)
_ZZZTUBE_DURATION_RE = re.compile(
...
_HQPORN_ANCHOR_RE = re.compile(
    r'<a\b(?P<attrs>[^>]*class="[^"]*\bjs-gallery-link\b[^"]*"[^>]*)>',
    re.IGNORECASE | re.DOTALL,
)
_HQPORN_URL_RE = re.compile(r'\bhref="(?P<url>/[^"]+\.html)"', re.IGNORECASE)
_HQPORN_TITLE_RE = re.compile(r'\btitle="(?P<title>[^"]+)"', re.IGNORECASE)
_HQPORN_PREVIEW_RE = re.compile(r'\bdata-preview="(?P<preview>https?://[^"]+\.mp4)"', re.IGNORECASE)
_HQPORN_THUMB_RE = re.compile(r'\bdata-src="(?P<thumb>(?:https?:)?//[^"]+)"', re.IGNORECASE)
_HQPORN_DURATION_RE = re.compile(
    r'class="[^"]*\bb-thumb-item__duration\b[^"]*"[^>]*>.*?'
    r'<span>\s*(?P<duration>[0-9]{1,3}:[0-9]{2}(?::[0-9]{2})?)\s*</span>',
    re.IGNORECASE | re.DOTALL,
)
...
_ANYPORN_URL_RE = re.compile(r'<a[^>]+href=["\'](?P<url>/[0-9]+/)["\']', re.IGNORECASE)
_ANYPORN_IMG_RE = re.compile(
    r'<img\b(?P<attrs>[^>]*class=["\'][^"\']*\bthumb\b[^"\']*["\'][^>]*)>',
    re.IGNORECASE | re.DOTALL,
)
_ANYPORN_THUMB_RE = re.compile(r'\bdata-original=["\'](?P<thumb>[^"\']+)["\']', re.IGNORECASE)
_ANYPORN_PREVIEW_RE = re.compile(r'\bdata-preview=["\'](?P<preview>[^"\']+)["\']', re.IGNORECASE)
_ANYPORN_TITLE_RE = re.compile(
    r'<strong[^>]+class=["\'][^"\']*\btitle\b[^"\']*["\'][^>]*>(?P<title>.*?)</strong>',
    re.IGNORECASE | re.DOTALL,
)
_ANYPORN_DURATION_RE = re.compile(
    r'innerHTML\s*=\s*["\'](?P<minutes>[0-9]+)m:(?P<seconds>[0-9]{1,2})s["\']',
    re.IGNORECASE,
)
...
_YOURLUST_URL_RE = re.compile(r'\bhref="(?P<url>/videos/[^"]+\.html)"', re.IGNORECASE)
_YOURLUST_TITLE_RE = re.compile(r'\btitle="(?P<title>[^"]+)"', re.IGNORECASE)
_YOURLUST_THUMB_RE = re.compile(
    r'<img\b[^>]*class="[^"]*\bthumb\b[^"]*"[^>]*\bsrc="(?P<thumb>https?://[^"]+)"',
    re.IGNORECASE | re.DOTALL,
)
_YOURLUST_DURATION_RE = re.compile(
    r'<div\b[^>]*class="length"[^>]*>\s*(?P<duration>[0-9]{1,3}:[0-9]{2}(?::[0-9]{2})?)\s*</div>',
    re.IGNORECASE | re.DOTALL,
)
...
def parse_yourlust_listing(raw_html: str, *, limit: int) -> list[SearchItem]:
    ...
    for match in _YOURLUST_CARD_RE.finditer(scope):
        block = match.group("body")
        href = _YOURLUST_URL_RE.search(block)
        title = _YOURLUST_TITLE_RE.search(block)
        thumb = _YOURLUST_THUMB_RE.search(block)
        duration = _YOURLUST_DURATION_RE.search(block)
        if href is None or title is None or thumb is None or duration is None:
            continue
        page_url = urljoin(base_url, html.unescape(href.group("url")))
        if page_url in seen:
...
def parse_anyporn_listing(raw_html: str, *, limit: int) -> list[SearchItem]:
    items: list[SearchItem] = []
    seen: set[str] = set()
    base_url = "https://anyporn.com/"
    for match in _ANYPORN_CARD_RE.finditer(raw_html):
        block = match.group("body")
        href = _ANYPORN_URL_RE.search(block)
        image = _ANYPORN_IMG_RE.search(block)
        title = _ANYPORN_TITLE_RE.search(block)
        duration = _ANYPORN_DURATION_RE.search(block)
        if href is None or image is None or title is None or duration is None:
            continue
        image_attrs = image.group("attrs")
        thumb = _ANYPORN_THUMB_RE.search(image_attrs)
        if thumb is None:
            continue
        page_url = urljoin(base_url, html.unescape(href.group("url")))
        if page_url in seen:
            continue
        seen.add(page_url)
        preview = _ANYPORN_PREVIEW_RE.search(image_attrs)
        items.append(
            SearchItem(
                id=_item_id("anyporn", page_url),
                provider="anyporn",
                title=_clean_text(title.group("title"))[:500],
                url=page_url,
                thumbnail=_https_media_url(thumb.group("thumb")),
                preview_url=(
                    _https_media_url(preview.group("preview")) if preview else None
                ),
                duration_seconds=(
                    int(duration.group("minutes")) * 60
                    + int(duration.group("seconds"))
                ),
                quality="HD" if "hdpng" in block.lower() else None,
                tags=[],
                score=1.0,
            )
        )
...
def parse_zzztube_listing(raw_html: str, *, limit: int) -> list[SearchItem]:
    ...
    for match in _ZZZTUBE_CARD_RE.finditer(raw_html):
        block = match.group("body")
        anchor = _ZZZTUBE_ANCHOR_RE.search(block)
        if anchor is None:
            continue
        attrs = anchor.group("attrs")
        href = _ZZZTUBE_URL_RE.search(attrs)
        title = _ZZZTUBE_TITLE_RE.search(attrs)
        thumb = _ZZZTUBE_THUMB_RE.search(block)
        duration = _ZZZTUBE_DURATION_RE.search(block)
        if href is None or title is None or thumb is None or duration is None:
            continue
        page_url = urljoin(base_url, html.unescape(href.group("url")))
        if page_url in seen:
            continue
        seen.add(page_url)
        preview = _ZZZTUBE_PREVIEW_RE.search(attrs)
        preview_url = preview.group("preview").strip() if preview else ""
        items.append(
            SearchItem(
                id=_item_id("zzztube", page_url),
                provider="zzztube",
                title=_clean_text(title.group("title"))[:500],
                url=page_url,
                thumbnail=_https_media_url(thumb.group("thumb")),
                preview_url=_https_media_url(preview_url) if preview_url else None,
                duration_seconds=_duration_clock(duration.group("duration")),
                quality="HD" if "icon-hd" in block.lower() else None,
                tags=[],
                score=1.0,
            )
        )
...
def parse_hqporn_listing(raw_html: str, *, limit: int) -> list[SearchItem]:
    items: list[SearchItem] = []
    seen: set[str] = set()
    base_url = "https://hqporn.xxx/"
    for match in _HQPORN_CARD_RE.finditer(raw_html):
        block = match.group("body")
        anchor = _HQPORN_ANCHOR_RE.search(block)
        if anchor is None:
            continue
        attrs = anchor.group("attrs")
        href = _HQPORN_URL_RE.search(attrs)
        title = _HQPORN_TITLE_RE.search(attrs)
        preview = _HQPORN_PREVIEW_RE.search(attrs)
        thumb = _HQPORN_THUMB_RE.search(block)
        duration = _HQPORN_DURATION_RE.search(block)
        if href is None or title is None or preview is None or thumb is None or duration is None:
            continue
        page_url = urljoin(base_url, html.unescape(href.group("url")))
        if page_url in seen:
...
class YourLustLiveAdapter(_HttpLiveAdapter):
    ...
    def _search_sync(self, query: str, *, page: int, limit: int) -> LiveProviderResult:
        ...
        if page > 1:
            url = f"{self.base_url}search/{page}/?{params}"
        raw = self._fetch_text(url)
        return LiveProviderResult(
            self.name, parse_yourlust_listing(raw, limit=limit), None, page,
            round((time.monotonic() - started) * 1000),
        )
...
class AnyPornLiveAdapter(_HttpLiveAdapter):
    name = "anyporn"
    base_url = "https://anyporn.com/"
    ...
    def _search_sync(self, query: str, *, page: int, limit: int) -> LiveProviderResult:
        ...
        raw = self._fetch_text(f"{self.base_url}search/{encoded}/")
        return LiveProviderResult(
            self.name, parse_anyporn_listing(raw, limit=limit), None, page,
            round((time.monotonic() - started) * 1000),
        )
...
class ZZZTubeLiveAdapter(_HttpLiveAdapter):
    ...
    def _search_sync(self, query: str, *, page: int, limit: int) -> LiveProviderResult:
        ...
        raw = self._fetch_text(url)
        return LiveProviderResult(
            self.name, parse_zzztube_listing(raw, limit=limit), None, page,
            round((time.monotonic() - started) * 1000),
        )
...
class HQPornLiveAdapter(_HttpLiveAdapter):
    name = "hqporn"
    base_url = "https://hqporn.xxx/"
    ...
    def _search_sync(self, query: str, *, page: int, limit: int) -> LiveProviderResult:
        ...
        raw = self._fetch_text(url)
        return LiveProviderResult(
            self.name, parse_hqporn_listing(raw, limit=limit), None, page,
            round((time.monotonic() - started) * 1000),
        )
