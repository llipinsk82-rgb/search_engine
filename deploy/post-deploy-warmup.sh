...
SYNC_WAIT_SECONDS="${SEARCH_WARMUP_SYNC_WAIT_SECONDS:-120}"

tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
curl -fsS --max-time 10 "$BASE_URL/api/stats" >"$tmp/before.json"

if ! systemctl start --no-block "$SYNC_SERVICE"; then
    echo "SEARCH_WARMUP=DEGRADED reason=sync-service-start-failed service=$SYNC_SERVICE" >&2
    exit 1
fi

finished=0
for _ in $(seq 1 "$SYNC_WAIT_SECONDS"); do
    state="$(systemctl show -p ActiveState --value "$SYNC_SERVICE")"
...
if [ "$finished" != 1 ]; then
    echo "SEARCH_WARMUP=QUEUED sync_service=$SYNC_SERVICE reason=sync-still-running wait_seconds=$SYNC_WAIT_SECONDS"
    exit 0
fi
if systemctl is-failed --quiet "$SYNC_SERVICE"; then
...
# Backfill is useful but not release-critical. Queue it and let the shared
# maintenance lock serialize it safely after the bounded sync.
if ! systemctl start --no-block "$SERVICE"; then
    echo "SEARCH_WARMUP=DEGRADED reason=backfill-service-start-failed service=$SERVICE" >&2
    exit 1
fi
...
print('SEARCH_WARMUP=PASS '
      f'items_before={before_items} items_after={after_items} '
      f"grown_providers={','.join(grown) or '-'} backfill=queued")
PY2
